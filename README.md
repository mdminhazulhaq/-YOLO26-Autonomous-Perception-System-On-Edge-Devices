...
#### 4th Stage: Run integrated (YOLO26 + Regression) model for object detection and distance calculation also compare predicted distance with GT distance
...
## Stage 4: 4th Stage: Run integrated (YOLO26 + Regression) model for object detection and distance calculation also compare predicted distance with GT distance

### How the Integrated Model Works

The final system combines two independently trained models:

```text
1. YOLO26n Object Detection Model
2. Distance Regression Model
```
...
(best_distance_model_13feature.pth)
              │
              ▼
Predicted Distance
              │
              ▼
Compare with GT Distance
```

### Step 1: YOLO26n trained 'best.pt' model first process the input image and predict object class, confidence score and boungin box coordination 
...
height   = 0.15
```
Then from predicted boxes, it calculate calculates the other following features: confidence, class_id, x_center, y_center, bottom_y, width, height, area, aspect_ratio, diagonal, inverse_height, inverse_area, scale_score.


### How Does YOLO26n tranined 'best.pt' model Predict Bounding Boxes?

During training of the YOLO26n model on images and labels fiels, the model actually tranined on images and labels files
...
The model learns the relationship between image pixels and object locations.

```text
Pixels → Class + Bounding Box
```

During inference, the model predicts new bounding boxes directly from unseen images. The model is not memorizing the label files; it is estimating object locations based on learned visual patterns. 

### Are the Predicted Bounding Boxes Exactly Equal to Ground Truth?
...
Small differences are expected because object detection is an estimation task.

The similarity between predicted and ground-truth bounding boxes is measured using Intersection over Union (IoU):

```text
IoU = Overlap Area / Union Area
```

A higher IoU indicates a more accurate detection.

### Step 2: Feature Extraction

The detected bounding box is converted into a feature vector for distance estimation.

For the final regression model, the following 13 features are used: confidence, class_id, x_center, y_center, bottom_y, width, height, area, aspect_ratio, diagonal, inverse_height, inverse_area, scale_score.


### Step 3: Distance Prediction

The extracted feature vector is passed to the trained distance regression model.
...
### Does Object Detection and Distance Prediction Run Simultaneously?
...
The regression model requires the bounding box information generated by YOLO26n, therefore distance prediction cannot begin until object detection is completed.
...
The integrated system consists of two models:
...
Total inference time can be expressed as:

```text
Total Time =
YOLO26n Detection Time
+
Distance Regression Time
```

For a single detected object:

```text
Regression Time ≈ Microseconds
```

For multiple detected objects (e.g., 50 objects):

```text
Regression Time ≈ A Few Milliseconds
```

Runtime distribution:

```text
YOLO26n Detection     ≈ 95–99%
Distance Regression   ≈ 1–5%
```

Therefore:

```text
Overall Latency ≈ YOLO26n Latency
```

The regression network is extremely lightweight:
...
As a result, the distance regression stage introduces negligible computational overhead and is suitable for real-time deployment.

Therefore, the regression model introduces negligible computational overhead.

### Sources of Distance Estimation Error

The final distance prediction error originates from two sources:

```text
1. Object Detection Error
2. Distance Regression Error
```

Object detection error occurs when the predicted bounding box differs slightly from the ground truth.

Distance regression error occurs when the regression model estimates a distance that differs from the actual object distance.
...
The same pipeline can be applied to video streams.
...
Distance Prediction
      │
      ▼
Visualization / ADAS Decision
```

Each frame is processed independently.

Because the regression network is extremely lightweight, real-time performance is primarily determined by the YOLO26n detector. The integrated system is therefore suitable for real-time object detection and monocular distance estimation on video sequences and edge-AI deployment platforms.
